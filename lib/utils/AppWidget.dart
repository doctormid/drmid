...
Widget text(
  ...
  return Text(
    ...
    style: TextStyle(
      ...
      fontSize: fontSize,
      color: textColor ?? UiHelper.onSecondary,
      height: 1.5,
...
BoxDecoration boxDecoration(
  ...
  return BoxDecoration(
    color: bgColor ?? UiHelper.background,
...
Future<List<LatLngAndGeohash>> getListOfLatLngAndGeoHash(BuildContext context) async {
  try {
    final fakeList = await (AppHelper.loadDataFromJson(context));
            ...
            children: <Widget>[
              ...
              Text(text,
                      style: primaryTextStyle(
                          size: textSize ?? 18, color: textColor ?? UiHelper.onPrimary))
                  .expand(),
            ],
          ).expand(),
          detail ?? Icon(Icons.arrow_forward_ios, size: 16, color: UiHelper.onSecondary),
        ],
...
Widget appBarTitleWidget(context, String title, {Color? color, Color? textColor}) {
  return Container(
    ...
    height: 60,
    color: color ?? UiHelper.primary,
    child: Row(
      children: <Widget>[
        Text(
          title,
          style: boldTextStyle(color: color ?? UiHelper.onPrimary, size: 20),
          maxLines: 1,
        ).expand(),
      ],
    ),
...
AppBar appBar(BuildContext context, String title,
  ...
  return AppBar(
    ...
    backgroundColor: color ?? UiHelper.primary,
    leading: showBack
        ? IconButton(
            onPressed: () {
              finish(context);
    ...
    return Card(
      ...
      child: ListTile(
        ...
        trailing: showTrailing
            ? Icon(Icons.arrow_forward_ios, size: 15, color: UiHelper.onPrimary)
